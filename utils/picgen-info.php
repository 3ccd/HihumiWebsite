<?php

/**
 * 文庫本フォーマットの画像をテキストファイルから生成
 * （生成ページ数のみ計算）
 */

    ini_set('display_errors', "On");
    define("FONT_SIZE", 45);
    define("FONT_LINE_SPACING", 50);
    define("FONT_DARKNESS", 0);
    define("FONT_OFFSET", 9);
    define("PAGE_PER_LINES", 17);
    define("PAGE_HEIGHT", 2894);
    define("PAGE_WIDTH", 2039);
    define("PAGE_HEADER", 350);
    define("PAGE_FOOTER", 300);
    define("PAGE_START", 120);

    function getPageInfo($mid, $cid, $cnum){

        $url = "../backnumbers/text/".$cid."_".$mid."_".$cnum.".txt";
        $text = file_get_contents($url);
        $text = mb_substr($text, 1);
        $l = mb_strlen($text,'UTF-8');
        $chunked = array();
        for ($i=0; $i<$l; $i++) {
            $chunked[] = mb_substr($text,$i,1,'UTF-8');
        }
        $chunks = count($chunked);

        $schar = 0;
        $output = array(0);
        $finFlag = false;
        for(;;){
            if($finFlag){
                break;
            }
            for($i = $schar; ; $i++){
                if($i >= $chunks -1){
                    $finFlag = true;
                    break;
                }
                if(RenderCharactor($chunked[$i], $chunked[$i+1],false) == 1){
                    $output[] = $i + 1;
                    $schar = $i +1;
                    RenderCharactor($chunked[$i], $chunked[$i+1], true);
                    break;
                }
            }
        }

        return $output;
    }
    
    function RenderCharactor($char, $nextChar, $init){
        static $line = 0;
        static $prevCharX = PAGE_START;
        static $height = 0;
        static $bboxHeight = PAGE_HEIGHT - PAGE_HEADER - PAGE_FOOTER;
        global $white, $gray, $font, $img;
        static $charX = 0;

        if($init){
            $line = 0;
            $prevCharX = PAGE_START;
            $height = 0;
            $charX = 0;
            return;
        }

        $charX = $prevCharX + FONT_SIZE + FONT_LINE_SPACING;
        $charY = $height + PAGE_HEADER;
        $height += FONT_SIZE + FONT_OFFSET;

        if($height >= $bboxHeight && strcmp($nextChar, "。") != 0 && strcmp($nextChar, "、") != 0){
            $line++;
            $height = 0;
            $prevCharX = $charX;
        }

        if(strcmp($char, "\n") == 0){
            $line++;
            $height = 0;
            $prevCharX = $charX;
        }

        if($line > PAGE_PER_LINES){
            return 1;
        }else{
            return 0;
        }
    }
?>
